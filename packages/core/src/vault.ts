...
async function applyWrites(fileStore: VaultFileStore, writes: VaultWrite[]): Promise<void> {
  ...
  try {
    for (const write of writes) {
      await fileStore.replace(write.previousPath, write.nextPath, write.markdown);
      applied.push(write);
    }
  } catch (error) {
    ...
    for (const write of applied.reverse()) {
      ...
      try {
        await fileStore.replace(write.rollback.previousPath, write.rollback.nextPath, write.rollback.markdown);
    ...
    if (rollbackError) {
      throw new Error('Vault save failed and rollback failed', { cause: rollbackError });
    }
    throw error;
  }
}
...
export function createMemoryVault(fileStore: VaultFileStore): MemoryVault {
  ...
  const saveDraft = async (draft: NoteDraft): Promise<MemoryNote> => {
    ...
    const writes: VaultWrite[] = [{
      ...
      markdown: serializeNote(candidate),
      rollback: previous?.path
        ? {
            previousPath: candidate.path,
            nextPath: previous.path,
            markdown: fileContents.get(previous.path) ?? serializeNote(previous),
          }
        : undefined,
    }];
    ...
    if (previous && previous.path !== candidate.path) {
      for (const note of nextNotes) {
        ...
        writes.push({
          ...
          nextPath: note.path,
          markdown: serializeNote(rewritten),
          rollback: {
            previousPath: note.path,
            nextPath: note.path,
            markdown: fileContents.get(note.path) ?? serializeNote(note),
          },
        });
      }
    }
    ...
    for (const { note, rewritten } of rewrittenNotes) {
      fileContents.set(note.path, serializeNote(rewritten));
      Object.assign(note, rewritten);
    }
